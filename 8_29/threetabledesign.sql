...
-- Disabling checking of foreign key constraint; not a good practice
-- SET FOREIGN_KEY_CHECKS=0;

-- Instead, create tables in the correct order.
-- Referenced tables need to be created before the table that has foreign keys referencing them.
DROP TABLE IF EXISTS `works`;
DROP TABLE IF EXISTS `dept`;
Drop table if exists emp;
 -- specific order (because of references in works, dept, and emp)
 --  ' ' is for reserved terms
